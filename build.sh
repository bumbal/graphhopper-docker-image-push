...
if [ "$1" == "--push" ]; then
  push="true"
  echo "${DOCKERHUB_TOKEN}" | docker login --username "${DOCKERHUB_USER}" --password-stdin || exit $?
  #docker login --username "${DOCKERHUB_USER}" --password-stdin <<< "${DOCKERHUB_TOKEN}" || exit $?
  shift
else
  push="false"
fi
...
imagename="bumbal/graphhopper:${1:-latest}"
if [ "$1" ]; then
